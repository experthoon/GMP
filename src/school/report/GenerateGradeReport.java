...
public class GenerateGradeReport {
	...
	public void getScoreGrade(Student student, Subject subject){
		...
		GradeEvaluation[] gradeEvaluation = {new BasicEvaluation(), new MajorEvaluation(), new PassFailEvaluation()};  //학점 평가 클래스들 각각 0, 1, 2 들어감
		...
		for(int i=0; i<scoreList.size(); i++){  // 학생이 가진 점수들 
			...
			if(score.getSubject().getSubjectId() == subject.getSubjectId()) {  // 현재 학점을 산출할 과목 
				...
				if(subject.getGradeType() == Define.PF_TYPE) { //PassFailEvaluation 부분 추가
